...
mapd2xnor: GPS source adapted from sunnypilot's `liveLocationKalman` to xnor's
`gpsLocationExternal` (xnor's locationd publishes only `livePose`, not the
deprecated liveLocationKalman). The mapd binary itself is GPS-source-agnostic —
it only reads the `LastGPSPosition` param this bridge writes — so swapping the
location stream here is sufficient; no C++ locationd port is required.
"""
...
class BaseMapData(ABC):
  def __init__(self):
    ...
    # mapd2xnor: xnor GPS source is gpsLocationExternal (ublox), not liveLocationKalman
  ...
  def publish(self) -> None:
    ...
    mapd_sp_send.valid = bool(self.sm['gpsLocationExternal'].hasFix)
