...
| **Speed-limit zone set-once, no auto-restore** | Entering a lower posted speed-limit zone sets the cruise speed **once** — the same percentage above the new limit the driver was running before (**at or under the old limit: exactly the new limit**, owner 2026-09-24, `limitdropexact2pnw`; never below the posted limit) — then stops managing it: no memory of the old set, no restore when the limit rises again. Curves **inside** the zone still restore to the zone-set speed, not the pre-zone speed. The one exception: a **police**-only slowdown still restores in full. | Driver directive: cruise should never creep back up after a limit-driven slowdown on its own; only a cleared police warning should give the speed back. | rides `AutoSpeedReduce` mode 2 (stock ACC) / op-long cap | ✅ | 2026-09-13 |
