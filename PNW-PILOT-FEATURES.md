...
| **Cruise cancels if our own SET/RES overshoots** | If openpilot's own SET−/RES tap brings the truck's stock cruise set back more than 3 mph above what was asked for, cruise is cancelled outright and steering drops too, with the alert "Cruise set too high - cancelled." A driver's own cruise-button press in the window is never cancelled. | Found live: a gas-set SET− made the PCM engage at 55 mph from 34 mph actual with a 42 mph memory (`drives/2026-09-13/corvallis-resume-55/`). Owner decision, option (a): cancel rather than leave the set running high. | `EventName.madsResumeSetTooHigh` (log.capnp `@104`) | ✅ | 2026-09-13 |
