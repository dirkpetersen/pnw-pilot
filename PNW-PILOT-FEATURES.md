...
| **PNW maps default + auto-download** | Default coverage **WA / OR / ID**; the first download auto-arms on a fresh deploy. | The device lives in the Pacific Northwest — no settings-page fiddling needed. | `OsmStateName` ("WA,OR,ID"), `MapdPnwMapsRequested`, `OsmAutoRequested`, `OsmDbUpdatesCheck` | ✅ | 2026-06-23 |
| **"Get map for this location"** | On-demand region download under current GPS, with a covered/greyed indicator. | Pull coverage for a trip outside the default states. | `GetMapForLocation`, `OSMDownloadLocations`/`OSMDownloadBounds` | ✅ | 2026-06-23 |
