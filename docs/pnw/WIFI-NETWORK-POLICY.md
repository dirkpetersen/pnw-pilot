---
updated: 2026-09-14          # audited against origin/3devpnw @ ed66cc6e04 (networkd/uploader last touched f65fdbdaf9)
status: current
---
...
`f65fdbdaf9`). Where an older doc (`docs/NETWORK2XNOR.md`) or a memory note disagrees with this code,
...
(`POLL_INTERVAL_S = 20.0`, `network_arbiterd.py:65`) and — **only while `TetheringEnabled=1`** — ranks
...
reasons until the pin ends. With `TetheringEnabled=0` the arbiter only ever tears the hotspot down if
...
| **The comma's own hotspot** | NM connection id literally `Hotspot` (`HOTSPOT_CONNECTION_ID`, `network_arbiter.py:50`). Broadcasts SSID `weedle-<first 4 chars of DongleId>` (`wifi_manager.py:188-192`), default password `"swagswagcomma"` (`DEFAULT_TETHERING_PASSWORD`, `wifi_manager.py:36`), gateway `192.168.43.1` (`TETHERING_IP_ADDRESS`, `wifi_manager.py:34`), subnet `192.168.43.0/24` (`TETHERING_SUBNET`/`HOTSPOT_SUBNET`, `wifi_manager.py:35`, `network_arbiterd.py:72`). Raising it installs `ip_forward` + an iptables-legacy MASQUERADE of that subnet out the LTE uplink (`_set_hotspot_nat`, `network_arbiterd.py:392-410`; the UI toggle's own copy is `_set_tethering_nat`, `wifi_manager.py:846-868`) — NAT is installed *before* the AP comes up so the first client packet already routes. | Param `TetheringEnabled` (`params_keys.h:57`, `PERSISTENT, BOOL, default "0"`) |
| **LTE** | The modem, `wwan0`, a **netplan-managed** gsm profile named `lte`. Never touched with `nmcli con modify` here — that crashes NetworkManager on this AGNOS (keyfile-writer assertion on a netplan-owned profile); only `con up`/`con down` and modem-level `mmcli` calls are used (`network_arbiterd.py:28-32`, `_park_lte`/`_unpark_lte`, `network_arbiterd.py:590-607`). | Params `GsmApn`, `GsmRoaming`, `GsmMetered` (`params_keys.h:53-55`) |
...
client wifi" (`network_arbiter.py:585-589`; tests `test_off_hotspot_up_tears_it_down`,
...
`network_arbiterd.py:101-142`, called only while tethering is off, `network_arbiterd.py:940-941`) — see
...
is not a synonym for either end (`network_arbiter.py:412-426`):

| Value | Meaning | Constant |
|---|---|---|
| `no` | asserted cheap (driver, or the OS, said so) | `COST_UNMETERED = 0` (`network_arbiter.py:413`) |
| `unknown` | **the default — nobody ever said** | `COST_UNKNOWN = 1` (`network_arbiter.py:414`) |
| `yes` | asserted expensive | `COST_METERED = 2` (`network_arbiter.py:415`) |
...
(`network_arbiterd.py:178-181`). **Folding `unknown` into `unmetered` is explicitly rejected** — an
earlier cut did that and let an alphabetical tiebreak decide between a metered Starlink and an
unmetered iPhone (`network_arbiter.py:454-457`; test `test_unknown_metered_state_is_NOT_treated_as_metered`,
...
`network_arbiterd.main()` (`network_arbiterd.py:736-1230`) runs forever, polling `nmcli` every
`POLL_INTERVAL_S = 20.0` s (`network_arbiterd.py:65`, each `nmcli` call bounded by
`NMCLI_TIMEOUT_S = 15.0` s, `network_arbiterd.py:66`). Each tick it builds a snapshot — active
connection, scan results, saved connections, metered states, GPS, the manual pick — and calls the pure
function `decide(...)` (`network_arbiter.py:541-625`), then applies exactly one action.

### The ranking, exactly (`choose_wifi`, `network_arbiter.py:429-500`)

For **every saved WiFi profile currently in range** (in the scan, or the currently-active usable link —
"sticky", `network_arbiter.py:483-486`), sort by:

1. **cost class** — `COST_UNMETERED (0) < COST_UNKNOWN (1) < COST_METERED (2)` (`network_arbiter.py:413-426`)
2. **configured membership** — a `TetheringPriorityNetworks` entry beats a non-member; members keep
   the driver's own list order among themselves (`member_rank`, `network_arbiter.py:475-480`)
3. **SSID** — a stable tiebreak so two equal-cost, equal-membership networks never flap
   (`network_arbiter.py:498-499`; test `test_the_choice_is_stable_between_equal_cost_networks`)
...
`network_arbiter.py:601-607`; both do the same `nmcli con up`, the name is purely about membership for
logging. `explain_fallback` (`network_arbiter.py:506-538`) computes, purely from `decide()`'s own
...
**Nothing usable → the comma's own hotspot** (`up_hotspot`, `network_arbiter.py:622-625`) — bridged
...
0 = ladder ON) is re-read every tick (`network_arbiterd.py:785`). Set, it reverts to the **exact
pre-ladder binary behaviour**: the first reachable *configured* entry, or the hotspot, no notion of
cost at all — deliberately, "that is what the kill switch is for" (`network_arbiter.py:579-583`). The
...
(`on_client_wifi = current_active not in (None, Hotspot)`, `network_arbiterd.py:882`). Disconnected or
sitting on the comma's own hotspot, scanning is **never** suppressed by GPS — a stale "far from home"
fix must never strand the device with no way to find WiFi again (`network_arbiterd.py:877-882`).
...
`network_arbiterd.py:68,855-873`). A `mobile: true` entry (the iPhone) never gets a learned location —
...
(`_read_gps`, `network_arbiterd.py:474,478-522`; 8 unit tests in `test_network_gps_freshness.py`).
...
`_carry_parked_fix`, `network_arbiterd.py:525-559` (`gpscarry2pnw`).
...
(`network_arbiter.py:147-176`) forces through **one scan per `UPGRADE_SCAN_S = 120.0` s**
(`network_arbiter.py:80`) whenever the ladder is on, we're on client WiFi, the active link is **not
explicitly unmetered**, no manual pin is in force, and the link is not still settling (a bring-up
awaiting judgement, or one that just appeared — so the very first upgrade scan can't land inside a
DHCP window). Suppressed entirely on an explicitly-unmetered link, since nothing can outrank it on
cost. Test: `test_the_upgrade_scan_is_throttled`; `test_no_scans_at_all_while_on_a_priority_network`.
...
`FAIL_BACKOFF_S = (60.0, 300.0, 900.0)` (`network_arbiterd.py:237`) — escalating, held at the 15-minute
...
driveway" (`network_arbiterd.py:238-241`). A network absent from `ABSENT_SCANS_FOR_FRESH_START = 2`
(`network_arbiterd.py:304`) **consecutive real scans** and then seen again gets a clean slate
(`_forget_on_reappearance`, `network_arbiterd.py:307-337`) — a scan that did not run (geo-gate
...
`False`=associated with none, `None`=could not tell, `network_arbiterd.py:253-301`) plus NM's per-device
...
`wwan0`'s: metric 600 vs 1000, `network_arbiterd.py:276`). `portal` state is treated as usable **only**
...
`DHCP_GRACE_S = 60.0` s (`network_arbiterd.py:232`) — NM's own DHCP timeout is 45 s and the poll is
20 s, so a first look can legally land mid-activation; inside the grace window a not-yet-usable link
stays sticky rather than being blamed. `judge_link` (`network_arbiter.py:328-409`) is the pure function
...
`ACTIVE_UNREADABLE_HOLD_S = 120.0` s (`network_arbiterd.py:250` — chosen as 2× `DHCP_GRACE_S`, "so an
...
(`seen_active or requested_active`, `network_arbiterd.py:1104`) — so a cold boot with nothing active yet
...
window as the hold above, so both release together (`network_arbiterd.py:977-984`; commit `f65fdbdaf9`).
...
| Event | Fires when |
|---|---|
| `network_arbiter_active_changed` | The active WiFi connection changed since the last read, including **NetworkManager's own** autoconnect switch — `by=arbiter`\|`external`\|`not_as_requested` (`network_arbiterd.py:797-813`, `smallfix0914pnw`) |
| `netcosttier_upgrade_scan` | An upgrade scan (§ above) was issued |
| `netcosttier_assoc_failed` / `netcosttier_recovered` / `netcosttier_ledger_cleared` | Failure-ledger transitions (`network_arbiterd.py:340-358,330-332`) |
...
| `network2xnor_lte_signal` | LTE bars/dBm/operator changed (throttled to every `SIGNAL_EVERY_N=3` ticks ≈ 60 s, `network_arbiterd.py:67`) |
...
action is suppressed (`network_arbiterd.py:1059-1078`) and logged once as
...
pick racing an ending old one can never be clobbered (`network_arbiterd.py:760-767`). While a pin is
**in force**, `decide()`'s ladder result is overridden to `noop` for any `up_priority`/`up_fallback`/
`up_hotspot` (`network_arbiterd.py:1080-1089`) — this holds through **both** the join itself (so the
...
### Every way a pin ends (`judge_pin`, `network_arbiter.py:107-144`)

| Reason | Exact condition |
|---|---|
| `failed` | `judge_link` blamed the pinned network (associated, no usable link, past `DHCP_GRACE_S`) — "a pin must never hold the device offline"; the failure ledger then applies as normal (`network_arbiter.py:117-119`) |
| `dropped` | It **was** the active link (seen active at least once since the pick) and no longer is |
| `join_timeout` | Never became the active link within `PIN_JOIN_WINDOW_S = 90.0` s of the arbiter first seeing the pick (`network_arbiter.py:77-79`) — chosen against NM's 45 s DHCP timeout plus the UI password-retry path re-adding the profile |
| `home` | A **stationary, explicitly unmetered** configured network has *arrived* since the pick (see below) |
| `superseded` | A newer `(ssid, ts)` pick replaced this one — decided by the daemon comparing identities (`network_arbiterd.py:835-840`) |
| `param_removed` | `WifiManualPick` went absent (SSH release hatch), **or** is present but the value could not be decoded as JSON — `params_pyx` returns `None` for both, so this reason covers both (`network_arbiterd.py:841-846`) |
...
advances a pin (`judge_pin`, `network_arbiter.py:132-133`; test `test_an_UNREADABLE_active_read_is_no_evidence`
...
is a silent 'no pin'" (`network_arbiterd.py:826-846`).

### Why a mobile network (the iPhone) never ends a pin

`home_to_yield_to` (`network_arbiter.py:243-281`) only considers **stationary** configured entries
(`not e.get("mobile")`, `network_arbiterd.py:1026`). Exempting mobile entries was deliberate: "the
...
(`network_arbiter.py:250-257,276-281`).

"Arrived" (`update_home_arrival`, `network_arbiter.py:182-240`) is established by evidence only — a
...
- `PIN_HOME_ABSENT_SCANS = 3` (`network_arbiter.py:178`) consecutive **real** scans that do not list
...
- GPS more than `PIN_HOME_FAR_M = 2.0 × HOME_GEOFENCE_M = 500.0` m (`network_arbiter.py:179`) from the
...
force (`network_arbiterd.py:834-840`). Because KarlMoik is now pinned, `decide()`'s cost-driven
...
if it comes into range and outranks KarlMoik on cost (§ "why a mobile network never ends a pin" above).
...
- `at_home` — the `OnPriorityNetwork` param (`params_keys.h:201`, `CLEAR_ON_MANAGER_START, BOOL`),
  ...
  active_metered)` (`network_arbiter.py:284-306`, `network_arbiterd.py:929-932`): **True** iff the
...
`network_arbiterd.py:276`). `map-data.pfeifer.dev` (mapd's download host) and `gitlab.com` (the
...
or you tap a different network). To force it: tap a different network (supersedes it immediately), or
remove the `WifiManualPick` param directly (the documented "manual release hatch over SSH",
`network_arbiter.py:462`) — `Params().remove("WifiManualPick")`. It also clears on any manager restart
...
- **Should a pin end when the *pinned* network is marked metered, or when an unmetered network
  appears?** Neither happens today. `judge_pin`'s `home` reason requires the *arriving* network to be
  explicitly unmetered and stationary (§4) — it says nothing about the pinned network's own metered
  state changing mid-pin, and there is no "an unmetered network merely appeared" trigger independent of
  the home-arrival evidence rule. **Not determined from code** whether this is an intentional design
  choice or an unconsidered case; no commit message addresses it directly.
...
|---|---|---|
| `POLL_INTERVAL_S` | 20.0 s | `system/networkd/network_arbiterd.py:65` |
| `NMCLI_TIMEOUT_S` | 15.0 s | `system/networkd/network_arbiterd.py:66` |
| `SIGNAL_EVERY_N` | 3 ticks (≈60 s) | `system/networkd/network_arbiterd.py:67` |
| `LEARN_MIN_MOVE_M` | 50.0 m | `system/networkd/network_arbiterd.py:68` |
| `PORTAL_MAX_TRIES` | 6 attempts/session | `system/networkd/network_arbiterd.py:69` |
| `HOTSPOT_SUBNET` | `192.168.43.0/24` | `system/networkd/network_arbiterd.py:72` |
| `DHCP_GRACE_S` | 60.0 s | `system/networkd/network_arbiterd.py:232` |
| `FAIL_BACKOFF_S` | (60.0, 300.0, 900.0) s | `system/networkd/network_arbiterd.py:237` |
| `ACTIVE_UNREADABLE_HOLD_S` | 120.0 s | `system/networkd/network_arbiterd.py:250` |
| `ABSENT_SCANS_FOR_FRESH_START` | 2 real scans | `system/networkd/network_arbiterd.py:304` |
| `GPS_MAX_AGE_S` | 10.0 s | `system/networkd/network_arbiterd.py:474` |
| `HOME_GEOFENCE_M` | 250.0 m | `system/networkd/geo_gate.py:18` |
| `PIN_JOIN_WINDOW_S` | 90.0 s | `system/networkd/network_arbiter.py:77` |
| `UPGRADE_SCAN_S` | 120.0 s | `system/networkd/network_arbiter.py:80` |
| `PIN_HOME_ABSENT_SCANS` | 3 real scans | `system/networkd/network_arbiter.py:178` |
| `PIN_HOME_FAR_M` | 500.0 m (2× `HOME_GEOFENCE_M`) | `system/networkd/network_arbiter.py:179` |
| `COST_UNMETERED` / `COST_UNKNOWN` / `COST_METERED` | 0 / 1 / 2 | `system/networkd/network_arbiter.py:413-415` |
...
| `wlan0`/`wwan0` IPv4 default-route metrics (measured, code comment) | 600 / 1000 | `system/networkd/network_arbiterd.py:276` |
...
| Mobile entry never ends a pin | `test_the_MOBILE_phone_in_range_does_NOT_end_the_pin` (`test_network_arbiter_sequences.py`) |
