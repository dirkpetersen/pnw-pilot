...
| `verify` | What the set speed actually came back at. `setHigher` (+`"loud":true`, + `cloudlog.error`) is the one that matters; `setLower` (also loud) means the PCM did *not* restore its remembered set — not dangerous, but not what this design assumes either; `noCruise` means the press produced no re-engagement at all (+`cloudlog.warning`, because that is also the signature of the executor being pinned without the matching panda safety gate). Since engagegoal2pnw a verify with `gotMs` also carries `driverBtn` (a driver cruise button between our press and the come-back) and `cancel`. **`cancel` is always false since 2026-09-14** (`nosetcancel2pnw`, owner: "remove it"): the >3 mph overshoot cancel is gone, and `overshootAction:"none"` says nothing acted on the verify. |
