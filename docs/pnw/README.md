...
| [CHANGELOG-2026-09-21.md](CHANGELOG-2026-09-21.md) | PNW changelog 09-21. **`capnpfork2pnw` SHIPPED (`674e0df124`, Fable APPROVE; pushed, NOT yet installed or device-verified):** the six fork `EventName`s → `OnroadEventsPnw @137`; the MADS reason → `PandaStatesPnw @138`; `healthPacketMismatch` → `@39` on the same bit; old logs migrated via the writer's own schema from git. `pandad` now links on the host. **`oplongtest2pnw`** (test-only): 2 tests red since 09-05, unseen. The Rule 9 gate never ran 7 of our `selfdrive/car/tests` files; now 3,655 green. Tumwater curve analysis: ICBM rejected a real 59.9 curve (inflated to 74.3). `SafetyModel` ordinal collision found (panda safety-mode IDs). |
