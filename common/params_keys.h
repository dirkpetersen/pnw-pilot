...
inline static std::unordered_map<std::string, ParamKeyAttributes> keys = {
    ...
    // netscanpin2pnw: the network the driver last picked BY HAND in Settings, as {"ssid": str, "ts": float}.
    // Written by the UI's WifiManager on every human join (saved-network tap AND password entry); read by
    // network_arbiterd, which treats that SSID as PINNED -- the cost ladder will not move the driver off it
    // while it is the active, usable network. The arbiter never writes it: a pin ends by the arbiter's own
    // judgement of the (ssid, ts) it last saw, so a new pick racing an ending pin cannot be clobbered.
    // CLEAR_ON_MANAGER_START: a pin never survives a reboot. Driver, 2026-09-13: "if you can identify that a
    // handpicked Wi-fi was selected then we want that to stick."
    {"WifiManualPick", {CLEAR_ON_MANAGER_START, JSON}},
    ...
    {"SteerLimitStatus", {CLEAR_ON_MANAGER_START, JSON}}, // steerlimit-log2pnw telemetry: PURE OBSERVATION per-tick steering-limit status (curvLim/safeLim/angDes/angAct/angErr/sat/latDem/latMax/curvMax, plus steertele2pnw's latActive/angSat) (controlsd -> CES event logger), ~5 Hz. mem-param, /dev/shm/params, same pattern as LaneCenterStatus. See docs/STEERING-LIMITS.md.
