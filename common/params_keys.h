...
inline static std::unordered_map<std::string, ParamKeyAttributes> keys = {
    ...
    {"OnPriorityNetwork", {CLEAR_ON_MANAGER_START, BOOL, "0"}},  // firehose2pnw: network_arbiterd sets True while connected to one of the priority (home / geo-gated) WiFi networks; lets the uploader run pass-2 (rlog/HD) even while onroad — an EV charging at home keeps ignitionLine on (onroad) and would otherwise block the best upload window
    {"LastUploadError", {CLEAR_ON_MANAGER_START, STRING}},  // firehose2pnw: uploader writes the last hard upload failure (HTTP status / exception + consecutive count) for the CES debug overlay; removed on the next successful upload
