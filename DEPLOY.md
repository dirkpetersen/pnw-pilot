...
| **Orphaned native `./pandad` holds the SPI panda → manager's pandad publishes nothing** | Running the python pandad manually (to debug) left an orphaned native `./pandad` claiming the SPI panda, so the manager-spawned one connected to nothing → `pandaStates` not alive, 0 CAN. Fix: `sudo pkill -9 -f pandad` ALL of them, GPIO-reset the panda, restart the stack once. Don't run pandad manually alongside manager. |
| **pandad stuck "Waiting for params to set safety model" → stays elm327** | `panda_safety.cc configureSafetyMode` only pushes the car safety model once `is_onroad && ControlsReady`. `ControlsReady` is set by `card.py:277` in `controls_update` (driven by `carControl` from controlsd). If card crashes (RCM_status bug) the whole chain stalls. After fixing card, a **clean single boot left alone** completes it → `safety=teslaLegacy`. |
| **★ `canBusMissing` / `carState canValid=False` even with `safety=teslaLegacy`, onroad, cruise available** | The port built Raven CAN parsers for a **7-bus** Tesla layout (`CANBUS.powertrain=4, chassis=5, autopilot_powertrain=6`), but a **comma 3X has only 3 physical buses (0/1/2)**. The `pt`(4)/`ap_pt`(6) parsers got no data → `canTimeout` → `canBusMissing`. **Live-confirmed on the 2021 Raven: ALL powertrain+chassis signals (DI_torque1=264, ESP_B=341, DI_state=872, BrakeMessage=522, DAS_control=697, DI_torque2=280) are on bus 1** in the `tesla_can` DBC. **Fix: in `carstate.get_can_parsers`, for `TESLA_MODEL_S_HW3` point `pt`/`ap_pt`/`chassis` all at the tesla_can DBC on bus 1.** (xnor reads the whole Raven from just buses 0+2 — its `CANBUS` has no powertrain/chassis lanes at all; our port inherited sunnypilot's modern multi-bus Tesla layout, which doesn't apply to the Raven.) |
| **★ "Communication Issue Between Processes" (sub: `radarState`, `longitudinalPlan`) on engage** | HW3 had `radarUnavailable = candidate in (CAR.TESLA_MODEL_S_HW2,)` = **False**, so openpilot thought the Tesla radar was available. But the comma harness does NOT tap the Tesla radar and `CANBUS.radar=5` doesn't exist on a 3-bus comma 3X → `RadarInterface.update()` polls a trigger msg that never arrives → returns `None` → `card.py:52 if RD is not None` skips publishing `liveTracks` → `radard`'s `radarState` goes stale → `plannerd`'s `longitudinalPlan` stalls → commIssue when engaged. **Fix: `interface._get_params_sx` set `ret.radarUnavailable = True` for the Raven** → `RadarInterface.update()` routes through `super().update(None)` which returns an empty `RadarData()` at 20 Hz → `card` publishes `liveTracks` steadily → pipeline alive (vision-only longitudinal). Verified `get_non_essential_params('TESLA_MODEL_S_HW3').radarUnavailable == True`. **NOTE: applied + isolation-verified, but NOT yet tested live while engaged.** |
