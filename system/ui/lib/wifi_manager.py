...
TETHERING_SUBNET = "192.168.43.0/24"  # network2pnw: AP client subnet, masqueraded out the LTE uplink
...
class WifiManager:
  ...
  def _set_others_autoconnect(self, enabled: bool):
    # network2pnw: the comma has a SINGLE wifi radio, so an AP (hotspot) and a client connection cannot
  ...
  def _set_tethering_nat(self, enabled: bool):
    # network2pnw: share the LTE uplink with tethered clients. Stock openpilot leaves NAT to NM's
    # `shared` mode and DISABLES ip_forward unless prime is NONE/LITE — but on this device NM adds NO
    # masquerade (legacy nat table empty) and PrimeType is -1 (unregistered fork), so clients got an IP
  ...
  def set_tethering_active(self, active: bool):
    # network2pnw: persist the user's intent so the hotspot survives reboot and so the arbitration
    # supervisor (system/networkd/network_arbiterd) knows whether tethering should be running.
    if Params is not None:
      try:
