...
def make_msg(face=True, pose_distracted=False, blink_distracted=False, phone_prob=0.0):
    ...
    # blink distraction — set BOTH API flavors so the test drives blink regardless of
    # which schema helpers.py reads:
    #   new API (bp-dev):   eyesClosedProb / eyesVisibleProb
    #   old API (xnor/bp-6.0): leftBlinkProb*rightBlinkProb gated by leftEyeProb/rightEyeProb/sunglassesProb,
    #                          averaged vs _BLINK_THRESHOLD
    left.eyesVisibleProb = 1.0
    left.eyesClosedProb  = 1.0 if blink_distracted else 0.0
    left.leftEyeProb  = 1.0
    left.rightEyeProb = 1.0
    left.sunglassesProb = 0.0
    left.leftBlinkProb  = 1.0 if blink_distracted else 0.0
    left.rightBlinkProb = 1.0 if blink_distracted else 0.0
