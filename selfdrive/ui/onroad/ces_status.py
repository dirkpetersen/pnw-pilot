...
class CesStatusRenderer(Widget):
  ...
  def _upload_err_line(self):
    """firehose2pnw: surface the uploader's last HARD failure (auth/permission/network — never the
    benign 'already there' 412) so a stuck upload is visible. Silent when clear; the uploader removes
    LastUploadError on the next successful upload."""
    try:
