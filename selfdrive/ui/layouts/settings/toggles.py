...
class TogglesLayout(Widget):
  ...
  def _update_toggles(self):
    ...
    # madsresume2pnw: the auto-resume needs the SAME two things "Disengage on brake" needs (the
    # capability + a MADS-flashed panda) plus the stock-ACC button path, since there is no brake-
    # induced lateral-only state without MADS and no RESUME button to tap without stock ACC. Grey it
